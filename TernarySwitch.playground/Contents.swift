...
func text<T>(a: (T,AnyObject)...) -> [(T,AnyObject)] {
    
    return a
    
}

life ??? [LifeStatus.Alive --> UIColor.greenColor(), LifeStatus.Dead --> UIColor.redColor(), LifeStatus.Zombie --> UIColor.grayColor()]


let color3 = life ??? [.Alive --> UIColor.greenColor(), .Dead --> UIColor.redColor(), .Zombie --> UIColor.grayColor()] ?? UIColor.whiteColor()
